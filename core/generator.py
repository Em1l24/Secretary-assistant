...
class DocumentGenerator:
    ...
    def _generate_vkr_documents(self, temp_dir, commission, chairman_reversed, secretary_reversed):
        ...
        for i, student in enumerate(students, start=5):
            ...
            # Используем общие данные если поля студента пустые
            context = {
                ...
                'initials_gene': to_genitive(fio),
                'theme': student.get('theme'),
                'chairman': commission.get('chairman'),
                'position': commission.get('chairman_position'),
                'member_1': commission.get('member_1', ''),
                'position_1': commission.get('member_1_position', ''),
                'member_2': commission.get('member_2', ''),
                'position_2': commission.get('member_2_position', ''),
                'member_3': commission.get('member_3', ''),
                'position_3': commission.get('member_3_position', ''),
                'member_4': member_4,
                'position_4': position_4,
                'secretary_1': secretary_reversed,
                'secretary_2': secretary_reversed,
                'secretary_3': commission.get('secretary'),
                ...
                'con': student.get('con'),
                'order': student.get('order') or common_vkr.get('order'),
                'pages': student.get('pages'),
                'con_1': student.get('con_1'),
                'con_2': student.get('con_2'),
                'review': student.get('review'),
                'time': student.get('time'),
                'questions_1': student.get('questions'),
                'property_2': student.get('property'),
                'point': student.get('point'),
                'initials_acc': to_dative(fio),
                'quali': student.get('quali') or common_vkr.get('quali'),
                'diplom': student.get('diplom'),
                'tom': student.get('tom'),
    ...
    def _generate_gos_documents(self, temp_dir, commission, chairman_reversed, secretary_reversed):
        ...
        # Получаем общие данные для госэкзамена (единые для ФИЭБ и Экзамена)
        ...
        for i, student in enumerate(test_students, start=4):
            ...
            context = {
                ...
                'property': student.get('property'),
                'initials': normalize_fio(fio),
        ...
        for i, student in enumerate(exam_students, start=4):
            ...
            context = {
                ...
                'initials_gen_1': to_accusative(fio),
                'ticket': student.get('ticket'),
                'state_date': student.get('state_date') or common_gos.get('state_date'),
                'questions': student.get('questions'),
                'add_questions': student.get('add_questions'),
                'property_1': student.get('property'),
                'initials_1': normalize_fio(fio),
                'mark_1': student.get('mark'),
                'chairman_1': chairman_reversed
            }
